...
class LabelPrintForm(forms.Form):
    ...
    label_sheet = forms.ModelChoiceField(queryset=LabelSheet.objects.all(), label="Label sheet type", initial=0)
    skipped_labels = forms.IntegerField(initial=0, label="Number of labels to skip", min_value=0)
